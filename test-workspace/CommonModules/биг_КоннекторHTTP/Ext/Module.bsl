...
Функция GetJson(URL, ПараметрыЗапроса = Неопределено, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) Экспорт
	...
	//**akimov.v [04.09.2024 23:36:57] #
	//заменено
		// Возврат КакJson(ВызватьHTTPМетод(ТекущаяСессия, "GET", URL, ДополнительныеПараметры), ПараметрыПреобразованияJSON);	
	//на
	Ответ = КакJson(ВызватьHTTPМетод(ТекущаяСессия, "GET", URL, ДополнительныеПараметры), ПараметрыПреобразованияJSON);	
	ЛоггироватьОтветНаЗапрос(URL, Ответ);	
	Возврат	Ответ;
	///akimov.v [04.09.2024 23:36:57]	
...
Функция PostJson(URL, Json, ДополнительныеПараметры = Неопределено, Сессия = Неопределено) Экспорт
	...
	ПараметрыПреобразованияJSON = 
		ВыбратьЗначение(Неопределено, ДополнительныеПараметры, "ПараметрыПреобразованияJSON", Неопределено);
	//**akimov.v [04.09.2024 23:39:04] #
	//заменено
		//Возврат КакJson(ВызватьHTTPМетод(ТекущаяСессия, "POST", URL, ДополнительныеПараметры), ПараметрыПреобразованияJSON);
	//на
	Ответ = КакJson(ВызватьHTTPМетод(ТекущаяСессия, "POST", URL, ДополнительныеПараметры), ПараметрыПреобразованияJSON);	
	ЛоггироватьОтветНаЗапрос(URL, Ответ);	
	Возврат	Ответ;	
	///akimov.v [04.09.2024 23:39:04]	
...
Процедура ПодготовитьТелоЗапроса(ПодготовленныйЗапрос, Данные, Файлы, Json, ПараметрыЗаписиJSON)
	...
	//++akimov.v [04.09.2024 20:53:59] #
	ЛоггироватьИсходящийПакет(ПодготовленныйЗапрос, СтрокаJson);
	///akimov.v [04.09.2024 20:54:02]
...
//++akimov.v [04.09.2024 21:17:58] #
Процедура ЛоггироватьИсходящийПакет(ПодготовленныйЗапрос, СтрокаJson)
	
	ВестиЛогирование = биг_ОбщегоНазначенияКлиентСервер.ПолучитьЗначениеПоИмени("ВестиЛогированиеИнтеграции", Истина, Истина);
	
	МассивИсключений = биг_ОбщегоНазначенияКлиентСервер.ПолучитьМассивЗначенийПоИмени("МетодыИнтеграцииИсключенныеИзЛогирования");
	
	СтруктураURL	= РазобратьURL(ПодготовленныйЗапрос.URL);
	
	Если ВестиЛогирование И МассивИсключений.Найти(СтруктураURL.Путь) = Неопределено Тогда  
		
		ТочкаСобытия			= биг_ОбщегоНазначенияКлиентСервер.ТекущаяДатаВМиллисекундахЭпохаUnixGMT0();
		
		Набор = РегистрыСведений.биг_ЛогиИнтеграций.СоздатьНаборЗаписей();
		Набор.Отбор.Сервер.Установить(СтруктураURL.Сервер);
		Набор.Отбор.Путь.Установить(СтруктураURL.Путь);
		Набор.Отбор.Метод.Установить(ПодготовленныйЗапрос.Метод);
		Набор.Отбор.ТочкаСобытия.Установить(ТочкаСобытия);
		
		Запись					= Набор.Добавить();
		Запись.Сервер			= СтруктураURL.Сервер;
		Запись.Путь 			= СтруктураURL.Путь;
		Запись.Метод			= ПодготовленныйЗапрос.Метод;
		Запись.ТочкаСобытия		= ТочкаСобытия;
		
		Запись.ВремяСобытия		= биг_ОбщегоНазначенияКлиентСервер.TimestampВДату(Запись.ТочкаСобытия/1000);		
		Запрос 					= "";
		РазбитьСтрокуПоРазделителю(Запрос, ПодготовленныйЗапрос.URL, "?", Истина);		
		Запись.ТелоСообщения	= ?(ЗначениеЗаполнено(СтрокаJson), СтрокаJson, Запрос);
		
		Набор.Записать(Истина);		
	КонецЕсли;
	
КонецПроцедуры ///akimov.v [04.09.2024 21:17:58]

//++akimov.v [04.09.2024 21:17:58] #
Процедура ЛоггироватьОтветНаЗапрос(URL, Ответ) Экспорт
	...
	Если ВестиЛогирование И МассивИсключений.Найти(СтруктураURL.Путь) = Неопределено Тогда
		...
		Набор.Отбор.Сервер.Установить(СтруктураURL.Сервер);
		Набор.Отбор.Путь.Установить(СтруктураURL.Путь);
		Набор.Отбор.Метод.Установить("ANSWER");
		Набор.Отбор.ТочкаСобытия.Установить(ТочкаСобытия);
		...
		Запись.ВремяСобытия		= биг_ОбщегоНазначенияКлиентСервер.TimestampВДату(Запись.ТочкаСобытия/1000);		
		Запись.ТелоСообщения	= ОбъектВJson(Ответ);
