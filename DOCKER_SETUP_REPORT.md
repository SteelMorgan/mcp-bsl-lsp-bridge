...
### Нормальный запуск “под проект” через `.env` (рекомендуется)
...
Шаги:
1) Скопируй `env.example` → `.env` (под конкретный проект) и отредактируй значения.
2) Собери и подними контейнер:
...
Имя контейнера будет: `mcp-lsp-${MCP_PROJECT_NAME}` (префикс задаётся `MCP_CONTAINER_PREFIX`).
